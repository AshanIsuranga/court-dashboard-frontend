...
@Component({
  ...
  styleUrl: './manage-courts.component.css'
})
...
onDistrictChange(selectedDistrict: string | null): void {
  ...
  fetchAllCourtDetails(province: string = this.selectProvince, district: string = this.selectDistrict, search: string = this.searchText) {
      this.isLoading = true;
      this.coreSrv.getCourtDetails(this.currentPage, this.itemsPerPage, province, district, search).subscribe(
          (res) => {
  ...
  addCourt() {
      this.router.navigate([`/courts/add-a-court`]);
  }
